...
import (
	...
	hhelper "github.com/m3ng9i/go-utils/http"
)
...
func serveFile(w http.ResponseWriter, r *http.Request, abspath string, setLastModified bool) error {
	...
	if err != nil {
		if f2, err2 := os.Open("index.html"); err2 == nil {
			http.ServeContent(w, r, "index.html", lastModified, f2)
		} else {
			return err2
		}
	}
...
func (this *RanServer) serveHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// display 404 error
	if !context.exist {
		if context2, err2 := indexContext(this.config); err2 == nil {
			context = context2
		} else {
